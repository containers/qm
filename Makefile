...
rpm: clean dist ##             - Creates a local RPM package, useful for development
	...
	rpmbuild -ba \
		--define="enable_qm_dropin_img_tempdir ${EN_QM_DROP_IMG_TMPDIR}" \
		--define="enable_qm_window_manager ${EN_QM_WINDOW_MGR}" \
		--define="enable_qm_mount_bind_tty7 ${EN_QM_MNT_BIND_TTY7}" \
		--define="enable_qm_mount_bind_ttyUSB0 ${EN_QM_MNT_BIND_TTY_USB}" \
		--define="enable_qm_mount_bind_sound ${EN_QM_MNT_BIND_SOUND}" \
		--define="enable_qm_mount_bind_kvm ${EN_QM_MNT_BIND_KVM}" \
		--define="enable_qm_mount_bind_input ${EN_QM_MNT_BIND_INPUT}" \
