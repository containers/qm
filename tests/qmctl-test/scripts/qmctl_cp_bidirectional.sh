...
verify_copy_content() {
    ...
    if [[ "$dst_path" == */ ]]; then
        local src_filename
        src_filename=$(basename "$4")  # $4 is the source path
    ...
    if [[ "$actual_dst_path" == qm:* ]]; then
        ...
        set +e
        actual_content=$(python3 "$QMCTL_SCRIPT" exec cat "$container_path" 2>/dev/null)
        ...
        # Remove 'output: ' prefix that qmctl adds
        if [[ "$actual_content" == "output: "* ]]; then
...
if [ $TEST_RESULTS -eq 0 ]; then
    ...
    fail_message "Some comprehensive qmctl copy tests failed"
    exit 1
fi