...
run_test "JSON Show container" 0 "json_valid" "" \
...
run_test "JSON Show shared-memory" 0 "json_valid" "" \
    python3 "$QMCTL_SCRIPT" show shared-memory --json

run_test "JSON Show available-devices" 0 "json_valid" "" \
    python3 "$QMCTL_SCRIPT" show available-devices --json

run_test "JSON Show namespaces" 0 "json_valid" "" \
...
if [ $TEST_RESULTS -eq 0 ]; then
    ...
    fail_message "Some qmctl JSON output tests failed"
    exit 1
fi